...
public class SystemMonitorWindow extends JFrame implements ActionListener {
	...
	/**
	 * A getter function for the CPU graph. Your threads can use this to add
	 * data points to the CPU graph.
	 * 
	...
	/**
	 * Adds rows to the process list. Call removeAllRowsFromProcList prior to
	 * adding new rows when updating the process list. This is definitely not
	 * the most efficient way to update this table, but it works. It also
	 * explains why the list may appear to 'jump' if you're scrolling while
	 * updating
	 * 
	 * @param data
	 */
	...
	public synchronized void removeAllRowsFromProcList() {
		// Apparently, the given code below that is commented below does not
		// work - I have received this information from piazza and other
		// students
		//
	...
	/**
	 * This helper function takes the memory information as parameters and
	 ...
	 *            The writeback
	 */
	public synchronized void updateMemoryInfo(int totalMem, int memFree,
			int memActive, int memInactive, int swapTotal, int swapFree,
			int dirtyPages, int writeback) {
	...
	public SystemMonitorWindow() {
		...
		// Creating the System Graph. Don't forget that you need to make the
		// number
		// of lines dynamic, I'd recommend passing in a variable.
		CPUGraph = new SysGraph(0, 60, 0, 100, numCores + 1, 500);
		...
		MemoryInfo.setBorder(BorderFactory
				.createTitledBorder("Memory Information"));
		...
		ProcInfo.setBorder(BorderFactory
				.createTitledBorder("Process Information"));

		// Initializing the Table to hold our data for proc info.
		String[] columnNames = { "NAME", "PID", "STATE", "# THREADS",
				"VOL_CTXT_SW", "NONVOL_CTXT_SW" };
	...
	public synchronized void actionPerformed(ActionEvent arg0) {
		// This function handles our menu click event. My code here is based off
		// of how I implemented
		// the interval code, if you do it differently, make appropriate
		// changes.
